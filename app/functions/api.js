...
handlers.deleteimg = function (info, req, res) {
  ...
  // Do the real work
  database.deleteImageNew(req.session.user.userid, info.imageid, function (
    ok,
    err
  ) {
    if (err) {
...
handlers.imageexists = function (info, req, res) {
  if (!req.isAuthenticated()) {
    ...
    // database.imageExists(req.user.userid, info.title, function (exists) {
    //   res.send(exists);
    // });
  }
};
...
handlers.saveimage = function (info, req, res) {
  ...
  if (!req.isAuthenticated()) {
    ...
    // do image checks
    // save image into user's image array

    // create the image object
    let image = new Image({
      imagesName: info.title,
      userId: req.user._id,
      code: String,
      ratings: 0,
      createdAt: Date(),
    ...
    // check that the user does not already own an image by the same name
  ...
  // } else {
  //   var query =
  //     "SELECT imageid FROM images WHERE title='" +
  ...
  //       } else {
  //         var newQuery =
  //           "UPDATE images SET code='" +
  //           database.sanitize(info.code) +
  //           "', modifiedAt= UTC_TIMESTAMP WHERE imageid=" +
  //           rows[0].imageid;
  ...
  //     else {
  //       var newQuery =
  //         "INSERT INTO images (userid, title, code, codeVisible, license, public, modifiedAt, createdAt) VALUES (" +
  //         req.session.user.userid +
  //         ",'" +
  //         database.sanitize(info.title) +
  //         "','" +
  //         database.sanitize(info.code) +
  //         "','" +
  //         database.sanitize(info.codeVisible) +
  //         "','" +
  //         database.sanitize(info.license) +
  //         "','" +
  //         database.sanitize(info.public) +
  ...
  //         } else {
  //           database.query(
  //             "SELECT imageid FROM images WHERE userid=" +
  //               req.session.user.userid +
  //               " AND title='" +
  //               info.title +
  //               "';",
...
handlers.deletews = function (info, req, res) {
  ...
  // Build the query
  var query =
    "DELETE FROM workspaces WHERE userid=" +
    req.session.user.userid +
    " and name='" +
    name +
    "'";
...
handlers.getws = function (info, req, res) {
  if (!req.session.user) {
    ...
    var query =
      "SELECT data FROM workspaces WHERE userid=" +
      req.session.user.userid +
      " and name='" +
...
handlers.listchallenges = function (info, req, res) {
  ...
  var query =
    "SELECT challenges.id, challenges.name, challenges.title, challenges.code FROM challengecategories,challenges WHERE challengecategories.description='" +
    category +
    "' and challengecategories.id = challenges.categoryid ORDER BY challenges.position;";
  database.query(query, function (rows, error) {
    if (error) {
      res.send(error);
    } else {
...
handlers.listws = function (info, req, res) {
  if (!req.session.user) {
    ...
    var query =
      "SELECT name FROM workspaces WHERE userid=" + req.session.user.userid;
    database.query(query, function (rows, error) {
      if (error) {
        fail(res, "Error: " + error);
      } // if error
  ...
  if (!req.session.user) {
    ...
    database.query(query, function (rows, error) {
      if (error) {
        ...
        var newQuery =
          "INSERT INTO workspaces (userid, name, data) VALUES (" +
          req.session.user.userid +
          ",'" +
          database.sanitize(info.name) +
          "','" +
...
handlers.omnisearch = function (info, req, res) {
  database.omnisearch(info.search, function (resultObject, error) {
    if (error) fail(res, JSON.stringify(resultObject));
    else res.end(JSON.stringify(resultObject));
  });
};
...
handlers.setFeatured = function (info, req, res) {
  ...
  if (info.state == "true") {
    database.addFeaturedImage(req.session.user.userid, info.imageid, function (
      response,
      err
    ) {
      if (err) {
...
handlers.toggleLike = function (info, req, res) {
  if (!req.session.user) fail(res, "User is not logged in.");
  else
    database.toggleLike(req.session.user.userid, info.imageid, function (
      success,
      error
    ) {
      if (error) fail(res, "Error: " + error);
      else res.end(success.toString());
    });
}; // handlers.toggleLike
...
handlers.deleteComment = function (info, req, res) {
  if (!req.session.user) fail(res, "User Not logged in");
  database.deleteComment(req.session.user.userid, info.commentId, function (
    success,
    error
  ) {
    if (error) fail(res, JSON.stringify(error));
    else if (success) res.end("Comment " + info.commentId + " deleted.");
    else fail(res, "Unknown error");
  });
};
...
handlers.flagComment = function (info, req, res) {
  if (!req.session.user) fail(res, "User not logged in");
  database.flagComment(info.commentId, req.session.user.userid, function (
    success,
    error
  ) {
    if (error) fail(res, JSON.stringify(error));
    else if (success) res.end("Comment " + info.commentId + " flagged.");
    else fail(res, "Unknown error");
  });
};
