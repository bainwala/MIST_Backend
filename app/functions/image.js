...
var setFlags = function (commentArray, userID, database, callback) {
  if (commentArray.length == 0) callback([]);
  else {
    var errorsArray = [];
    ...
    for (var i = 0; i < commentArrayClone.length; i++) {
      (function (currentIndex) {
        database.hasFlaggedComment(
          userID,
          commentArrayClone[currentIndex].commentId,
          function (flagged, error) {
  ...
  database.imageInfo(req.params.imageid, function (image, error) {
    if (error) res.end(JSON.stringify(error));
      ...
      database.hasLiked(
        ...
        image.imageid,
        function (liked, error) {
          if (error) res.end(JSON.stringify(error));
          else {
            database.commentInfo(image.imageid, function (comment, error) {
              if (error) res.end(JSON.stringify(error));
              else if (req.session.user != null) {
                database.albumsInfo(req.session.user.userid, function (
                  albums,
                  error
                ) {
                  if (error) res.end(JSON.stringify(error));
...
module.exports.addtoAlbum = function (req, res, database) {
  database.addtoAlbum(req.body.add, req.params.imageid, function (
    success,
    error
  ) {
    if (!success) {
...
module.exports.deleteImage = function (req, res, database) {
  database.deleteImage(req.session.user.userid, req.params.imageid, function (
    success,
    error
  ) {
    if (!success) res.end(JSON.stringify(error));
    else res.redirect("/");
  });
};

module.exports.setProfilePicture = function (req, res, database) {
  database.setProfilePicture(
    req.session.user.userid,
    req.params.imageid,
    function (success, error) {
      if (!success) res.end(JSON.stringify(error));
      else res.redirect("/user/" + req.session.user.username + "/");
    }
  );
};
